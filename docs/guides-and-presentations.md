...
Local PNG, JPEG, GIF, and WebP images inside the Markdown source directory are embedded in the published HTML, including slide backgrounds. Keep each image below 2 MiB and the final HTML below the server's page limit. Unsupported local formats, images outside the source directory, CSS imports, and unresolved assets fail before publication. Do not put remote fonts, CDNs, or external image URLs in a deck; the CLI rejects the resulting HTML. PDF/PPTX exports require a supported local Chromium installation used by Marp.
