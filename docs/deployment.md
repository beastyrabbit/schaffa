...
The admin UI and API provide immediate page/file takedown and a publishing lockdown. Lockdown stops all new uploads and updates but intentionally leaves public reads, content deletion, token revocation, and the setting itself available for recovery. Admin logout only removes the eight-hour browser cookie; revoke the underlying token when it may be compromised.
