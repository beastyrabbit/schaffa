...
export function renderAdmin(input: {
  ...
  return layout(
    ...
    `<script defer src="/assets/admin-filters.js"></script>${input.newToken ? `<script defer src="/assets/token-setup.js"></script>` : ""}`,
  );
}
...
function userLabel(user: UserSummary): string {
  ...
  return user.name || user.email || `Shoo user · ${user.id.slice(0, 8)}`;
}
